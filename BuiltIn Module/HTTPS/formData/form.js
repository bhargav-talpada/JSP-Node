    ...
    if (req.method === "POST") {
        ...
        if (req.headers['content-type'] === formUrlEncoded) {
            ...
            req.on("end", () => {
                ...
                // querystring is third party module its provide parse method, parse -> convert string data into object format...
                let name = payload.name;
                let email = payload.email;
                let password = payload.password;
                console.log(payload)

                fs.appendFile("./form.txt",`name="${name}", email="${email}", password="${password}"`,(err) => {
                    if (err) {
